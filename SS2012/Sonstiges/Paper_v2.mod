...
%--------------------------------------------------------------------------
	...
	varobs Y C I pi w L R; % hier noch evtl. Anzahl der Var verändern
...
% Werte aus dem Paper von Smets and Wouters (S. 22 und Tabelle1)
    ...
    beta=0.99; % Discountfaktor
    adj=5.911 ; % adjustment cost of investment ???????- S=((e_I*I)/I(-1)) S(1)=0 im steady state muss dann auch noch irgendwo spezifiziert werden
    tau=0.025; % Abschreibungsrate
...
%--------------------------------------------------------------------------
	...
	R=rho*R(-1)+(1-rho)*(e_pi +r_pi*(pi(-1)-e_pi)+r_Y*Y)+r_dpi*(pi-pi(-1))+r_dY*(Y-Y(-1))+n_R;
...
stoch_simul(periods=10000,irf=30,order=1); % oder müssen hier die Daten eingelesen werden, bzw. diesen part weglassen und dafür unten bei datafile direkt auf daten beziehen (oben dann bei obs die richitgen angeben)
save('output1.mat','Y' 'C' 'I' 'pi' 'w' 'L' 'R');   % hier noch anpassen
...
estimation(datafile=output1, first_obs=1000 , nobs=100 , mh_replic=0, mode_check); 
estimation(datafile=output1, first_obs=1000 , nobs=100 , mh_replic=10000, mh_nblocks=1,mode_compute=0, mode_file=Paper_v2_mode, mh_jscale=0.3); 
steady;check;
