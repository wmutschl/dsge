...
@# if scenario == 7
    ...
    estimation(mode_compute=7,order=1,datafile=simdat,first_obs=201,nobs=500,mode_check);            
    steady; check; resid;
@# endif
...
@# if scenario == 8 
	...
	estimation(mode_compute=0,mode_file=rbc_mode,datafile=simdat,first_obs=401,nobs=200,mh_replic=5000,mh_nblocks=3,mh_jscale=1.7);
	steady; check; resid;
    
    % Run Metropolis-Hastings again without discarding previous draws
    estimation(mode_compute=0,mode_file=rbc_mode,datafile=simdat,first_obs=401,nobs=200,mh_replic=1000,mh_nblocks=3,load_mh_file,mh_jscale=1.7);
	steady; check;  resid;
@#endif