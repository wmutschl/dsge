...
%----------------------------------------------------------------
   ...
   @#endif
    ...
    @#if exercise == 9      
        ...
        estimation(order=1,datafile=simdat,nobs=50,first_obs=51,nograph);        
   @#endif
   ...
   @#if exercise == 10
       %Bayesian estimation
        ...
        estimation(order=1,datafile=simdat,first_obs=51,nobs=300,mode_check,mh_replic=1000,mh_jscale=1.5,mh_nblocks=1);
   @#endif
