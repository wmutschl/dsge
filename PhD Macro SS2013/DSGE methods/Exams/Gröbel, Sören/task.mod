...
% plug in defined priors as in the textfile.
	estimated_params;
	alpha, beta_pdf, .45, .05;
	theta, beta_pdf, .35, .05;
	tau, gamma_pdf, mode_and_variance_to_mean(2,.5^2,1,1),.5, 1;
	end;
	varobs y;


% Estimate posterior mode: only for checking posterior mode: see
% comments in the pdf solution sheet. 
 estimation(datafile=artificial_sample,nobs=200,mh_replic=0 ,mode_check);
 	steady; check;

% Run Metropolis-Hastings with already calculated posterior mode: with 
% possible changes and comments described in the solution sheet. 
	estimation(datafile=artificial_sample,first_obs=8001,nobs=200,mh_replic=5000,mh_nblocks=3,mh_jscale=1.7);
	
% Run Metropolis-Hastings again without discarding previous draws
    estimation(datafile=artificial_sample,first_obs=8001,nobs=200,mh_replic=1000,mh_nblocks=3,mode_compute=0,mode_file=task_mode,load_mh_file,mh_jscale=1.7);
	steady; check;
